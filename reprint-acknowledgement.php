...
<?php require_once ('../connection.php');
	...
	$testID=mysqli_real_escape_string($db, $_GET["testid"]);
 ...
 <!-- end of carosul
      ...
      <div class="row" id="ques2"><!-- end of row -->
             ...
             <table width="100%" align="center" id="ttt">
                <tr>
                     <td width="15%" align="left" style="padding-left:2px;"><div id="photos"><img src="<?php echo SITEURL; ?>/backend/logos/<?php echo $contents["logo"] ?>" width="100px" height="60px" style="border-radius:7px; box-shadow:2px 2px 1px 1px #CCCCCC;"></div></td>
					...
					<div id="photos">
						<?php if($content2["passport"] != ""){ ?>
								<img src="<?php echo SITEURL; ?>/backend/uploads/studentpassport/<?php echo $content2["passport"] ?>"  class="img-circle profile_img">
						<?php } else {?>
							<img src="<?php echo SITEURL; ?>/backend/images/user.png"  class="img-circle profile_img">
						<?php } ?>
					</div></td>
                 </tr>
			...
			<?php         
                ///////////////////////////////////////////
				$select_content42=("select * from quiz_result q INNER JOIN students s ON q.sid=s.student_id where sid= '$stid' and sub_id='$subject_id' and testID='$testID'");
				$content_result42= mysqli_query($db, $select_content42) or die(mysqli_error($db));
				$content42 = mysqli_fetch_assoc($content_result42);
				$score = $content42["score"];
				$dates = $content42["date_written"];
            ?>              
             <img src="../backend/logos/<?php echo $contents["logo"] ?>" style="z-index:-1000; width:1000px; height:900px; position:absolute; top:250px; right:120px; opacity:0.2; filter:alpha(opacity=12);"> 
           <table width="100%" style="color:rgba(0,0,102,1);" >
             
             <tr>
               <td colspan="3" align="center"><h3><?php echo $content42["surname"]." ".$content42["othername"].' ('.$class.')'; ?> </h3><font color="#FF0000">Test ID: <?php echo $testID; ?></font></td>
             </tr>
             <tr>
			<?php         
                ///////////////////////////////////////////
				$select_content41=("select subjects, sesion from pans p INNER JOIN subjects t ON p.term_id=t.tid INNER JOIN schsession s ON p.sessionid=s.sid where p.userid= '$stid' and p.subject_id='$subject_id' and p.testID='$testID'");
          			...
          			<tbody>    
						<?php         
							...
							do { 
								...
								if($ans == $student_ans){
									$ans1 = $ans. 2;
									$select_content12=("select question from quiz_question where id='$qid'");
									$status = true;
								}
								else{
									$ans1 = $ans. 2;
                      ...
                      <tr>
                        <td align="center">
         				...
         				<td>&nbsp;<?php echo $content12[$ans] ?></td>
           				<td align="center" id="ques"<?php if($content2['ans']==$content2['student_ans']){?> style="background:#6F0; text-align:center; color:#FFF" <?php } else{?> style="background:#F00; text-align:center; color:#FFF"<?php } ?>>
   	...
   	<table width="100%" align="center" id="ttt">
    	<tr>
        	<td  colspan="2" align="left" style="padding-left:15px; color:#FFF">
            	 <a href="http://noraktech.com" target="_blank" style="color:#FFF"></a>
            </td> 
